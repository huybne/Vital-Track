...
static esp_err_t sim800_power_down(modem_dce_t *dce)
{
    ...
    // Bước 1: Đặt hàm xử lý dòng trả về cho lệnh `AT+CPOWD=1`
    dce->handle_line = sim800_handle_power_down;

    // Bước 2: Gửi lệnh `AT+CPOWD=1` để tắt nguồn
    if (dte->send_cmd(dte, "AT+CPOWD=1\r", MODEM_COMMAND_TIMEOUT_POWEROFF) != ESP_OK)
    {
        ESP_LOGE(DCE_TAG, "Failed to send power down command");
        return ESP_FAIL;
    }

    // Bước 3: Kiểm tra trạng thái trả về
    if (dce->state != MODEM_STATE_SUCCESS)
    {
        ESP_LOGW(DCE_TAG, "Modem did not respond with POWER DOWN message. Trying recovery...");

        // Thử gửi lệnh "AT" để đồng bộ
        dce->handle_line = esp_modem_dce_handle_response_default;
        if (dte->send_cmd(dte, "AT\r", MODEM_COMMAND_TIMEOUT_DEFAULT) != ESP_OK)
        {
            ESP_LOGE(DCE_TAG, "Failed to sync modem after power down failure");
            return ESP_FAIL;
        }

        if (dce->state != MODEM_STATE_SUCCESS)
        {
            ESP_LOGE(DCE_TAG, "Modem sync failed after power down attempt");
            return ESP_FAIL;
        }
    }

    ESP_LOGI(DCE_TAG, "Modem powered down successfully");
    return ESP_OK;
}


static esp_err_t sim800_wakeup(modem_dce_t *dce)
{
    modem_dte_t *dte = dce->dte;
    // Gửi lệnh AT để đánh thức
    DCE_CHECK(dte->send_cmd(dte, "AT\r", MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_OK, "send command failed", err);
    // Kiểm tra trạng thái
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "wakeup failed", err);
    ESP_LOGD(DCE_TAG, "SIM800 woke up successfully");
    return ESP_OK;
err:
    ESP_LOGE(DCE_TAG, "Failed to wake up SIM800");
    return ESP_FAIL;
}
/**
...
static esp_err_t sim800_set_working_mode(modem_dce_t *dce, modem_mode_t mode)
{
    ...
    case MODEM_COMMAND_MODE:
        ...
        vTaskDelay(pdMS_TO_TICKS(5000)); // spec: 1s delay for the modem to recognize the escape sequence
...
modem_dce_t *sim800_init(modem_dte_t *dte)
{
    ...
    esp_modem_dce->parent.wakeup = sim800_wakeup; // Thêm wakeup ở đây
