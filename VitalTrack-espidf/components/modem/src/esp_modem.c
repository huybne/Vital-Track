...
static esp_err_t esp_modem_dte_change_mode(modem_dte_t *dte, modem_mode_t new_mode)
{
    ...
    ESP_LOGI(MODEM_TAG, "In change mode");
    switch (new_mode) {
    case MODEM_PPP_MODE:
        ESP_LOGI(MODEM_TAG, "Changing to ppp mode");
    ...
    case MODEM_COMMAND_MODE:
        ESP_LOGI(MODEM_TAG, "Changing to command mode");
        MODEM_CHECK(dce->set_working_mode(dce, new_mode) == ESP_OK, "set new working mode:%d failed", err_restore_mode, new_mode);
...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
        res = uart_set_hw_flow_ctrl(esp_dte->uart_port, UART_HW_FLOWCTRL_CTS_RTS, UART_FIFO_OVF - 8);
    } else if (config->flow_control == MODEM_FLOW_CONTROL_SW) {
        res = uart_set_sw_flow_ctrl(esp_dte->uart_port, true, 8, UART_FIFO_OVF - 8);
    }
...
esp_err_t esp_modem_start_ppp(modem_dte_t *dte)
{
    ...
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
    /* Set PDP Context */
    MODEM_CHECK(dce->define_pdp_context(dce, 1, "IP", "v-internet") == ESP_OK, "set MODEM APN failed", err);
    /* Enter PPP mode */
